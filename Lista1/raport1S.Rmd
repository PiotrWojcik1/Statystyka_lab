...
column1 <- c(var(Estimators1Data1), varEstimator(Estimators1Data1, 1), BiasEst(Estimators1Data1, 1))
column2 <- c(var(Estimators2Data1), varEstimator(Estimators2Data1, 1), BiasEst(Estimators2Data1, 1))
column3 <- c(var(Estimators3Data1), varEstimator(Estimators3Data1, 1), BiasEst(Estimators3Data1, 1))
column4 <- c(var(Estimators4Data1), varEstimator(Estimators4Data1, 1), BiasEst(Estimators4Data1, 1))
...
rows <- c("Wariancja", "MSE", "Obciążenie")
column1 <- c(var(Estimators1Data2), varEstimator(Estimators1Data2, 4), BiasEst(Estimators1Data2, 4))
column2 <- c(var(Estimators2Data2), varEstimator(Estimators2Data2, 4), BiasEst(Estimators2Data2, 4))
column3 <- c(var(Estimators3Data2), varEstimator(Estimators3Data2, 4), BiasEst(Estimators3Data2, 4))
column4 <- c(var(Estimators4Data2),varEstimator(Estimators4Data2, 4), BiasEst(Estimators4Data2, 4))
table <- data.frame(column1, column2, column3, column4)
row.names(table) <- rows
knitr::kable(table, row.names = TRUE, escape = FALSE, format = "pipe", align = "c", col.names = c("$\\hat{\\theta_1}$", "$\\hat{\\theta_2}$", "$\\hat{\\theta_3}$", "$\\hat{\\theta_4}$"))
```
...
rows <- c("Wariancja", "MSE", "Obciążenie")
column1 <- c(var(Estimators1Data3), varEstimator(Estimators1Data3, 1), BiasEst(Estimators1Data3, 1))
column2 <- c(var(Estimators2Data3), varEstimator(Estimators2Data3, 1), BiasEst(Estimators2Data3, 1))
column3 <- c(var(Estimators3Data3), varEstimator(Estimators3Data3, 1), BiasEst(Estimators3Data3, 1))
column4 <- c(var(Estimators4Data3), varEstimator(Estimators4Data3, 1), BiasEst(Estimators4Data3, 1))
table <- data.frame(column1, column2, column3, column4)
row.names(table) <- rows
knitr::kable(table, row.names = TRUE, escape = FALSE, format = "pipe", align = "c", col.names = c("$\\hat{\\theta_1}$", "$\\hat{\\theta_2}$", "$\\hat{\\theta_3}$", "$\\hat{\\theta_4}$"))
...
```
<p>Następnie oszacujemy parametr przesunięcia $\small \theta$, metodą Newtona.</p>
...
first_logistic_derivatice <- function(x, t, s) return(length(x)/s -2*sum(nexp(x,t,s)/(s*(1+nexp(x,t,s)))))
second_logistic_derivatice <- function(x, t, s) return(-2/s^2*sum(nexp(x,t,s)/(1+nexp(x,t,s))^2))
```

```{r test_on_trial, echo = FALSE, warning = FALSE, tidy = TRUE}

Newtons_method(LogistTrial2, first_logistic_derivatice, second_logistic_derivatice, s = 1, i = 10)
Newtons_method(LogistTrial3, first_logistic_derivatice, second_logistic_derivatice, s = 2, i = 10)

LogistTrial1 <- rlogis(50, 1, 1)
est <- Newtons_method(LogistTrial1, first_logistic_derivatice, second_logistic_derivatice, s = 1, i = 10)
mle(LogistTrial1, est, 1)
mle(LogistTrial1, est-0.01, 1)
mle(LogistTrial1, est+0.01, 1)

LogistTrial2 <- rlogis(50, 4, 1)
est <- Newtons_method(LogistTrial2, first_logistic_derivatice, second_logistic_derivatice, s = 1, i = 10)
mle(LogistTrial2, est, 1)
mle(LogistTrial2, est-0.01, 1)
mle(LogistTrial2, est+0.01, 1)

LogistTrial3 <- rlogis(50, 1, 2)
est <- Newtons_method(LogistTrial3, first_logistic_derivatice, second_logistic_derivatice, s = 2, i = 10)
mle(LogistTrial3, est, 2)
mle(LogistTrial3, est-1, 2)
mle(LogistTrial3, est+1, 2)

plot(seq(1,2, by = 0.01), sapply(0:100, function(x) mle(LogistTrial3, (1 + x/100), 2)))
lines(x = c(est, est), y = c(-500,500), type = "l", col = "red")


```

```{r}
mle <- function(x,t,s) return(sum(-(x-t)/s - log(s) - log(1+2*nexp(x,t,s)+exp(-2*(x-t)/s))))
```