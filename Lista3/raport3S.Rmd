...
<p>Podsumowując powyższe wyniki można dojść do wniosku, że nasze testy nie są bardzo czułe na zmianę próby analizując prawdopodobieństwo pokrycia. Choć zachodzą nieznaczne poprawy przy wzroście próby dla wszystkich rozkładów przy estymowaniu średniej. W przypadku wariancji, tylko przy rozkładach normalnych można zauważyć poprawę, co nie jest zaskakujące, gdyż problem słabego pokrycia wariancji w pozostałych rozkładach, nie wynikał z rozmiaru próby. Warto też zwrócić uwagę, że wraz ze wzrostem próby przedziały ufności znacznie zmieniejszały swoje długości.</p>