...
<h2>Zadanie 1</h2>
<p>Załóżmy, że chcemy skonstruować przedział, do którego z prawdopodobieństwej $\small 1 - \alpha$ będzie należeć średnia $\small \mu$ z rozkładu normalnego $\small N(\mu, \sigma^2)$, gdzie $\small \sigma^2$ jest znane, mając do dyspozycji pewną próbę $\small X1, \ldots , X_n$ z tego rozkładu. Weźmy średnią $\small \overline{X}$ tej próby, wtedy jej rozkład jest postaci $\small N(\mu, \frac{\sigma^2}{n})$. Po ustandaryzowaniu dostajemy:</p>
$$
...
```
<p>Wyniki jakie uzyskaliśmy są dość oczekiwane zważając na dobrany poziom istotności równy 95%. Jedyny rozkład który znacznie odbiega od teoretycznej wartości jest rozkład <em>Cauchy'ego</em>.Spowodowane jest to tym, że w odróżnieniu od pozostałych rozkładów, ten nie zbiega do standardowego normalnego z <em>Centalnego twierdzenia granicznego</em>. We wszystkich przypadkach możemy zaobserwować, że długości przedziałów rosną wraz ze wzrostem parametru $\small \sigma$, co nie powinno zaskakiwać, skoro wraz ze wzrostem tego parametru, wzrasta rozrzut danych w rozkładzie normalnym.</p>
...
$$
<p>dostajemy zmienną o rozkładzie studenta z $\small n - 1$ stopniami swobody. Oznacza to, że jedyna różnica w porównaniu z poprzednim zadaniem jest w doborze kwantyli $\small t_{\alpha/2}$ i $\small t_{1-\alpha/2}$ z rozkładu studenta z $\small n-1$ stopniami swobody, zamiast kwantyli z rozkładu normalnego. Ostatecznie więc, nasz przedział ufności jest postaci:</p>
$$
...
<h2>Zadanie 7</h2>
<p>W poniższym zadaniu skonstruujemy analogiczny przedział ufności co do tego w zadaniu 5, ale bez znanej średniej $\small \mu$. Zmienna $\small S^2$ ma rozkład</p>
$$
...
knitr::kable(table, row.names = TRUE, escape = FALSE, format = "pipe", align = "c", col.names = c("prawdopodobieństwo pokrycia", "średnia długość przedziałów ufności"))
```
<p>Wyniki oczywiście są analogiczne do tych jakie uzyskaliśmy w zadaniu 6, ponieważ konstrukcja wykorzystana tutaj, korzysta z tego samego faktu co w konstrukcji z zadania 6.</p>
...
$$
<p>ma w przybliżeniu rozkład normalny dla dużych $\small n$(asymptotycznie normalny rozkład). Stąd już możemy wyprowadzić asymptotyczny przedział ufności dla proporcji $\small p$ na poziomie ufności $\small \alpha - 1$ postaci:</p>
$$
...
```{r ex10data, echo = TRUE, warning = FALSE, tidy = TRUE}
...
$$
<p>Jak mogliśmy się spodziewać, nasze przedziały są skoncentorwane blisko $\small 0.95$, ponieważ wszystkie testy jakie wykonywaliśmy w zadaniu 2 były na poziomie ufność $0.95$. Są one bardzo wąskie, ale widać, że każdy kolejny jest szerszy od poprzedniego, ponieważ dla kolejnych rozkładów wariancja była większa.</p>
...
$$
<p>Jak w podpunkcie (a), nasze przedziały są skoncentorwane blisko $\small 0.95$, ponieważ wszystkie testy jakie wykonywaliśmy w zadaniu 2 były na poziomie ufność $0.95$. Są one bardzo wąskie, ale widać, że każdy kolejny jest szerszy od poprzedniego, ponieważ dla kolejnych rozkładów wariancja była większa.</p>
...
```{r ex10data20, echo = TRUE, warning = FALSE, tidy = TRUE}
...
```{r ex10data100, echo = TRUE, warning = FALSE, tidy = TRUE}
...
<p>Podsumowując wszystkie powyższe wyniki można dojść do wniosku, że nasze testy nie są bardzo czułe na zmianę próby analizując prawdopodobieństwo pokrycia. Choć zachodzą nieznaczne poprawy przy wzroście próby dla wszystkich rozkładów przy estymowaniu średniej. W przypadku wariancji, tylko przy rozkładach normalnych można zauważyć poprawę, co nie jest zaskakujące, gdyż problem słabego pokrycia wariancji w pozostałych rozkładach, nie wynikał z rozmiaru próby. Warto też zwrócić uwagę, że wraz ze wzrostem próby przedziały ufności znacznie zmieniejszały swoje długości. W zadaniu 10 warto zwrócić uwagę, że rozmiar próby nie miał znaczenia, gdyż wszystkie wyniki są równie analogiczne dla próby 20, 50 jak i 100 elementowej. Nie powinno to zaskakiwać, skoro przedziały ufności dla proporcji były wykonywane z wykorzystaniem próby 10 000 elmentowej dla wszystkich powyższych przypadków.</p>